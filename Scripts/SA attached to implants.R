...
## Summarise the data
df <- data_summary(sa, varname = "Log10.CFU.per.ml", groupnames = c("Implant.type"))
## Reorder fill (implant type) manually
df$Implant.type <- factor(df$Implant.type, levels = c("F", "E", "A", "G", "B", "D", "C"))
head(df)


## geom_errorbar() can be used to produce a bar graph with error bars
## Standard deviation of the mean as error bar
p <- ggplot(df, aes(x = Implant.type, y = Log10.CFU.per.ml, fill = Implant.type)) + 
  geom_bar(stat = "identity", color = "black", position = position_dodge()) + 
  geom_errorbar(aes(ymin = Log10.CFU.per.ml - sd, ymax = Log10.CFU.per.ml + sd), width = 0.4, position = position_dodge(0.9)) +
  scale_y_continuous(limits = c(0,6), expand = expand_scale(mult = c(0, 0.1)), breaks = seq(0,6, by = 1)) +
  scale_x_discrete(expand = c(0,0))

p + scale_fill_brewer(palette = "Paired") + theme_minimal() +
  labs(x = "Implant type", y = "", fill = "") + theme_classic() + theme(text = element_text(size = 15)) 
